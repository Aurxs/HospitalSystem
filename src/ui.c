...
static void ui_show_message_with_underlay(const char *title, const char *message, int type, WINDOW *underlay);
static int ui_confirm_dialog_with_underlay(const char *title, const char *message, WINDOW *underlay);
...
int ui_init(void) {
    ...
    if (g_users == NULL) {
        ...
        ui_show_message_with_underlay("安全提示", hint, 5, stdscr);
...
void ui_show_message(const char *title, const char *message, int type) {
    ui_show_message_with_underlay(title, message, type, stdscr);
}
...
int ui_confirm_dialog(const char *title, const char *message) {
    return ui_confirm_dialog_with_underlay(title, message, stdscr);
}
            ...
            case KEY_ENTER:
                if (current_field == 0) {
                    ...
                    if (strlen(username) == 0 || strlen(password) == 0) {
                        ui_show_message_with_underlay("错误", "请输入用户名和密码", 2, login_win);
                    } else {
                        /* 验证用户 */
                        if (authenticate_user(g_users, username, password)) {
                            ...
                            if (g_current_user == NULL) {
                                ui_show_message_with_underlay("错误", "登录状态异常，请重试", 2, login_win);
                                break;
                            }
                            ...
                            login_attempts++;
                            if (login_attempts >= 3) {
                                ui_show_message_with_underlay("警告", "登录失败次数过多，请稍后再试", 3, login_win);
                                login_attempts = 0;
                            } else {
                                ui_show_message_with_underlay("错误", "用户名或密码错误", 2, login_win);
                            }
                            /* 清空密码 */
            ...
            case KEY_ENTER:
                if (current_field < 6) {
                    ...
                    if (strlen(name) == 0 || strlen(phone) == 0) {
                        ui_show_message_with_underlay("错误", "姓名和电话为必填项", 2, form_win);
                    } else {
                        int age = 0;
                        if (!parse_int_in_range(age_str, 0, 150, &age)) {
                            ui_show_message_with_underlay("错误", "年龄必须是0-150的整数", 2, form_win);
                            break;
                        }
                        PatientNode p = make_patient(name, age, gender, phone, diagnosis, treatment);
                        add_patient(&g_patients, p);
                        save_patients(DATA_PATH_PATIENTS, g_patients);
                        ui_show_message_with_underlay("成功", "患者信息添加成功", 1, form_win);
                        delwin(form_win);
                        return 0;
                    }
                }
            ...
            case KEY_ENTER:
                ...
                if (ui_input_string(search_win, 4, 15, keyword, 25, 0) >= 0) {
                    ...
                    if (result != NULL) {
                        ...
                        ui_show_message_with_underlay("查询结果", msg, 1, search_win);
                    } else {
                        ui_show_message_with_underlay("查询结果", "未找到匹配的患者", 3, search_win);
                    }
                }
                break;
            case 27:
            ...
            case KEY_ENTER:
                switch (selected) {
                    case 0:
                        ...
                        save_patients(DATA_PATH_PATIENTS, g_patients);
                        ui_show_message_with_underlay("成功", "已按姓名排序", 1, menu_win);
                        break;
                    case 1:
                        g_patients = sort_patients_by_phone(g_patients);
                        save_patients(DATA_PATH_PATIENTS, g_patients);
                        ui_show_message_with_underlay("成功", "已按电话排序", 1, menu_win);
                        break;
                    case 2:
                        g_patients = sort_patients_by_age(g_patients);
                        save_patients(DATA_PATH_PATIENTS, g_patients);
                        ui_show_message_with_underlay("成功", "已按年龄排序", 1, menu_win);
                        break;
                }
                delwin(menu_win);
                return;
        ...
        switch (ch) {
            ...
            case 'D':
                if (total_count > 0) {
                    ...
                    if (p != NULL) {
                        if (ui_confirm_dialog_with_underlay("确认删除", "确定要删除该患者信息吗?", content_win)) {
                            ...
                            ui_show_message_with_underlay("成功", "患者信息已删除", 1, content_win);
                        }
                    }
                }
                break;
            ...
            case KEY_ENTER:
                if (current_field < 6) {
                    ...
                    if (strlen(name) == 0 || strlen(phone) == 0) {
                        ...
                        save_doctors(DATA_PATH_DOCTORS, g_doctors);
                        ui_show_message_with_underlay("成功", "医生信息添加成功", 1, form_win);
                        delwin(form_win);
                        return 0;
                    }
                }
            ...
            case KEY_ENTER:
                ...
                if (ui_input_string(search_win, 4, 15, keyword, 25, 0) >= 0) {
                    ...
                    if (result != NULL) {
                        ...
                        ui_show_message_with_underlay("查询结果", "未找到匹配的医生", 3, search_win);
                    }
                }
                break;
            case 27:
        ...
        switch (ch) {
            ...
            case 'D':
                if (total_count > 0) {
                    ...
                    if (d != NULL && ui_confirm_dialog_with_underlay("确认删除", "确定要删除该医生信息吗?", content_win)) {
                        g_doctors = delete_doctor(g_doctors, d->phone);
                        save_doctors(DATA_PATH_DOCTORS, g_doctors);
                        total_count = count_list_nodes(g_doctors, 1);
                        if (selected_row >= total_count) selected_row = total_count - 1;
                        if (selected_row < 0) selected_row = 0;
                        ui_show_message_with_underlay("成功", "医生信息已删除", 1, content_win);
                    }
                }
                break;
            case 'm':
        ...
        switch (ch) {
            ...
            case KEY_ENTER:
                if (current_field < 5) {
                    ...
                    if (strlen(name) == 0) {
                        ui_show_message_with_underlay("错误", "药品名称为必填项", 2, form_win);
                    } else {
                        double price = 0.0;
                        int stock = 0;
                        if (!parse_non_negative_double(price_str, &price)) {
                            ui_show_message_with_underlay("错误", "价格必须是非负数字", 2, form_win);
                            break;
                        }
                        if (!parse_int_in_range(stock_str, 0, INT_MAX, &stock)) {
                            ui_show_message_with_underlay("错误", "库存必须是非负整数", 2, form_win);
                            break;
                        }
                        DrugNode d = make_drug(name, spec, factory, price, stock);
                        add_drug(&g_drugs, d);
                        save_drugs(DATA_PATH_DRUGS, g_drugs);
                        ui_show_message_with_underlay("成功", "药品信息添加成功", 1, form_win);
                        delwin(form_win);
                        return 0;
                    }
                }
            ...
            case KEY_ENTER:
                ...
                if (ui_input_string(search_win, 2, 15, keyword, 25, 0) >= 0) {
                    ...
                    if (result) {
                        ...
                        ui_show_message_with_underlay("查询结果", "未找到匹配的药品", 3, search_win);
                    }
                }
                break;
        ...
        switch (ch) {
            ...
            case KEY_ENTER:
                if (selected == 0) {
                    ...
                    save_drugs(DATA_PATH_DRUGS, g_drugs);
                    ui_show_message_with_underlay("成功", "已按价格排序", 1, menu_win);
                } else if (selected == 1) {
                    g_drugs = sort_drugs_by_stock(g_drugs);
                    save_drugs(DATA_PATH_DRUGS, g_drugs);
                    ui_show_message_with_underlay("成功", "已按库存排序", 1, menu_win);
                }
                delwin(menu_win);
                return;
        ...
        switch (ch) {
            ...
            case 'D':
                if (total_count > 0) {
                    ...
                    if (d && ui_confirm_dialog_with_underlay("确认删除", "确定要删除该药品信息吗?", content_win)) {
                        g_drugs = delete_drug(g_drugs, d->name);
                        save_drugs(DATA_PATH_DRUGS, g_drugs);
                        total_count = count_list_nodes(g_drugs, 2);
                        if (selected_row >= total_count) selected_row = total_count - 1;
                        if (selected_row < 0) selected_row = 0;
                        ui_show_message_with_underlay("成功", "药品信息已删除", 1, content_win);
                    }
                }
                break;
            case 'm':
            ...
            case KEY_ENTER:
                if (current_field < 4) {
                    ...
                    if (strlen(patientName) == 0 || strlen(doctorName) ==
                        0) { ui_show_message_with_underlay("错误", "患者和医生姓名必填", 2, form_win); } else {
                        RegisterNode r = make_registration(patientName, doctorName, department, date);
                        add_registration(&g_registrations, r);
                        save_registrations(DATA_PATH_REGISTRATIONS, g_registrations);
                        ui_show_message_with_underlay("成功", "挂号成功", 1, form_win);
                        delwin(form_win);
                        return 0;
                    }
                }
            ...
            if (ui_input_string(w, 2, 15, kw, 25, 0) >= 0) {
                ...
                if (r) {
                    ...
                    snprintf(msg, sizeof(msg), "找到: %.20s -> %.20s, %.15s", r->patientName, r->doctorName,
                             r->department);
                    ui_show_message_with_underlay("结果", msg, 1, w);
                } else ui_show_message_with_underlay("结果", "未找到", 3, w);
            }
        } else if (ch == 27) {
            delwin(w);
            return;
        ...
        switch (ch) {
            ...
            case 'D':
                if (total > 0) {
                    ...
                    if (r && ui_confirm_dialog_with_underlay("确认", "删除此挂号?", content_win)) {
        ...
        switch (ch) {
            ...
            case KEY_ENTER:
                if (cf < 3) {
                    ...
                    if (strlen(pn) == 0) ui_show_message_with_underlay("错误", "患者姓名必填", 2, w);
                    else {
                        double a = 0.0;
                        if (!parse_non_negative_double(amt, &a)) {
                            ui_show_message_with_underlay("错误", "金额必须是非负数字", 2, w);
                            break;
                        }
            ...
            case KEY_ENTER:
                if (cf < 3) {
                    ...
                    if (g_current_user != NULL &&
                        ...
                        ui_show_message_with_underlay("错误", "医生只能修改自己患者的费用", 2, w);
                        break;
                    }
...
void ui_sort_bill_menu(WINDOW *parent_win) {
    ...
    save_bills(DATA_PATH_BILLS, g_bills);
    ui_show_message_with_underlay("成功", "已按金额排序", 1, parent_win);
}
        ...
        switch (ch) {
            ...
            case 'D':
                if (total > 0) {
                    ...
                    if (b && ui_confirm_dialog_with_underlay("确认", "删除此费用?", content_win)) {
        ...
        switch (ch) {
            ...
            case KEY_ENTER:
                if (cf == 0) {
                    ...
                    if (strlen(un) == 0 || strlen(pw) == 0) ui_show_message_with_underlay("错误", "用户名密码必填", 2, w);
                    else if (find_user(g_users, un)) ui_show_message_with_underlay("错误", "用户已存在", 2, w);
                    else {
                        AuthNode a = make_user(un, pw, role);
                        add_user(&g_users, a);
                        save_users(DATA_PATH_USERS, g_users);
                        ui_show_message_with_underlay("成功", "用户添加成功", 1, w);
                        delwin(w);
                        return 0;
                    }
                }
        ...
        switch (ch) {
            ...
            case KEY_ENTER:
                if (cf == 0) {
                    ...
                    if (strlen(pw) == 0) ui_show_message_with_underlay("错误", "密码不能为空", 2, w);
                    else {
        ...
        switch (ch) {
            ...
            case 'D':
                if (total > 0) {
                    ...
                    if (u && strcmp(u->username, "admin") != 0 && ui_confirm_dialog_with_underlay("确认", "删除此用户?", content_win)) {
                        g_users = delete_user(g_users, u->username);
                        save_users(DATA_PATH_USERS, g_users);
                        total = count_list_nodes(g_users, 5);
                        if (sel >= total) sel = total - 1;
                        if (sel < 0) sel = 0;
                    } else if (u && strcmp(u->username, "admin") == 0) ui_show_message_with_underlay("错误", "不能删除admin", 2, content_win);
                }
                break;
            case 'm':
            case 'M': if (total > 0) {
    ...
    while (1) {
        ...
        switch (ch) {
            ...
            case KEY_ENTER: {
                /* 选择医生进行挂号 */
                if (filtered_count == 0) {
                    ui_show_message_with_underlay("提示", "没有可选择的医生", 3, content_win);
                    break;
                }
                        ...
                        switch (confirm_ch) {
                            ...
                            case KEY_ENTER:
                                if (confirm_field == 0) {
                                    ...
                                    if (strlen(date) == 0) {
                                        ui_show_message_with_underlay("错误", "请输入挂号日期", 2, confirm_win);
                                    } else {
        ...
        if (ch == '\n' || ch == KEY_ENTER) {
            ...
            if (ui_input_string(w, 2, 11, kw, 25, 0) >= 0) {
                ...
                if (found) {
                    ...
                    ui_show_message_with_underlay("结果", "在您的患者中未找到", 3, w);
                }
            }
        } else if (ch == 27) {
            delwin(w);
        ...
        if (ch == '\n' || ch == KEY_ENTER) {
            ...
            if (ui_input_string(w, 2, 15, kw, 25, 0) >= 0) {
                ...
                if (found) {
                    ...
                    ui_show_message_with_underlay("结果", "在您的挂号中未找到", 3, w);
                }
            }
        } else if (ch == 27) {
            delwin(w);
        ...
        switch (ch) {
            ...
            case KEY_ENTER:
                if (cf < 3) {
                    ...
                    if (strlen(pn) == 0) {
                        ui_show_message_with_underlay("错误", "患者姓名必填", 2, w);
                    } else if (!is_patient_of_doctor(pn, g_current_user->username)) {
                        /* 检查是否是医生自己的患者 */
                        ui_show_message_with_underlay("错误", "该患者不是您的患者，无法添加费用", 2, w);
                    } else {
                        double a = 0.0;
                        if (!parse_non_negative_double(amt, &a)) {
                            ui_show_message_with_underlay("错误", "金额必须是非负数字", 2, w);
        ...
        switch (ch) {
            ...
            case 'D':
                if (my_count > 0) {
                    ...
                    if (sel_reg != NULL && ui_confirm_dialog_with_underlay("确认", "删除此挂号记录?", content_win)) {
    ...
    while (1) {
        ...
        switch (ch) {
            ...
            case 'A': {
                ...
                if (my_patient_count == 0) {
                    ui_show_message_with_underlay("提示", "您目前没有患者，无法添加费用", 3, content_win);
                } else {
            ...
            case 'D':
                if (my_count > 0) {
                    ...
                    if (sel_bill != NULL && ui_confirm_dialog_with_underlay("确认", "删除此费用记录?", content_win)) {
            ...
            case KEY_ENTER: {
                ...
                if (role == ROLE_PATIENT) {
                    ...
                    if (actual_menu == MENU_PATIENT_MGMT ||
                        ...
                        ui_show_message_with_underlay("权限不足", "患者账户无法访问此功能", 2, sidebar_win);
                        allowed = 0;
                    }
                } else if (role == ROLE_DOCTOR) {
                    /* 医生不能访问药品管理和用户管理 */
                    if (actual_menu == MENU_DRUG_MGMT || actual_menu == MENU_USER_MGMT) {
                        ui_show_message_with_underlay("权限不足", "医生账户无法访问此功能", 2, sidebar_win);
                        allowed = 0;
                    }
                }
